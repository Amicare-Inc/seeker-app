    ...
    return (
        ...
        >
            ...
            >
				...
				<TouchableOpacity onPress={handleToggle} activeOpacity={1} className="flex-row items-center">
                    <View className="flex-col px-5 pb-7 pt-3 flex-1">
						<Text className="text-white text-lg font-bold">
							Booking Sent:{' '}
							<Text className="font-normal">Awaiting Confirmation</Text>
                        </Text>
                        {!expanded && (
                ...
                {expanded && (
                    <>
						<View className="flex-row justify-between items-center rounded-full px-6 py-2.5 mb-4 mx-5 mt-2" style={{ backgroundColor: '#FFFFFF' }}>
							<View className="flex-row items-center">
								<Ionicons name="calendar-outline" size={24} color="#1F2937" />
								<Text className="ml-2 text-[17px] font-medium" style={{ color: '#1F2937' }}>{dateLabel}</Text>
								<Text className="text-xs ml-2" style={{ color: '#10B981' }}>{`${isNextDay ? '+1' : ''}`}</Text>
							</View>
							<View style={{ width: 1, height: 28, backgroundColor: '#E5E7EB' }} />
							<View className="flex-row items-center">
								<Ionicons name="time-outline" size={24} color="#1F2937" />
								<Text className="ml-2 text-[17px] font-medium" style={{ color: '#1F2937' }}>{timeRange}</Text>
							</View>
                        </View>
