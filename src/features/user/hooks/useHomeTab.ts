...
// Export useHomeTab as the main hook that wraps useHomeUsers
const useHomeTab = (isPsw: boolean, withDistance: boolean = false) => {
	// Get the fetched users using your existing hook.
	const { users: fetchedUsers, loading, error } = useAvailableUsers(isPsw, withDistance);
	// Get the current user ID.
	const currentUserId = useSelector(
		(state: RootState) => state.user.userData?.id,
	);
	...
	// Filter out users who are already engaged.
	const availableUsers = useMemo(() => {
		return fetchedUsers.filter((user: User) => !engagedUserIds.includes(user.id!));
	}, [fetchedUsers, engagedUserIds]);
