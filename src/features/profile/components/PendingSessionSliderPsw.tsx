...
const PendingSessionSliderPsw: React.FC<PendingSessionSliderProps> = ({
	session,
	onRequestChange,
}) => {
	const currentUser = useSelector((state: RootState) => state.user.userData);
	const sessionConfirmation = useSessionConfirmation(session.id, 'book');
	const rejectSessionMutation = useRejectSession();

	const handleAccept = async () => {
		try {
			await sessionConfirmation.uiContent?.onPrimaryPress?.();
			router.back();
		} catch (err) {
...
export default PendingSessionSliderPsw;
