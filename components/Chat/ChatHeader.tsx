...
const ChatHeader: React.FC<ChatHeaderProps> = ({ session, user, isExpanded, toggleExpanded }) => {
  ...
  const isNextDay = startDateObj && endDateObj ? endDateObj.getDate() !== startDateObj.getDate() : false;
      ...
      <View className="flex-row items-center px-4 py-3">
        ...
        {isExpanded && (
          <TouchableOpacity onPress={handleNavigateToUserProfile}>
            <Ionicons name="information-circle-outline" size={27} color={isConfirmed ? '#fff' : '#000'} />
          </TouchableOpacity>
        )}
      </View>
        ...
        <View className="px-4 pb-4">
          ...
          <View
            ...
            style={{
              borderColor: '#ffffff',
              width: '100%',
              backgroundColor: isConfirmed ? 'transparent' : '#f0f0f0',
            }}
          >
          ...
          <View className="flex-row items-center justify-between mb-3">
            <View className="flex-row items-center">
              <Text className={`text-xs ${isConfirmed ? 'text-white' : 'text-black'}`}>
                {isConfirmed ? 'Appointment Confirmed' : 'Awaiting confirmation'}
              </Text>
              <Ionicons
