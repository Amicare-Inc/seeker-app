...
const ProfileEditPanel: React.FC<ProfileEditPanelProps> = ({ user }) => {
  ...
  const [careTypeSelection, setCareTypeSelection] = useState<string[]>(user.carePreferences?.careType || []);
  const [tasksSelection, setTasksSelection] = useState<string[]>(user.carePreferences?.tasks || []);
  ...
  const isCareTypeChanged =
    careTypeSelection.join(",") !== (user.carePreferences?.careType || []).join(",");
  const isTasksChanged =
    tasksSelection.join(",") !== (user.carePreferences?.tasks || []).join(",");
  ...
  return (
    <View className="p-4">
      ...
      {isAnyChanged && (
        ...
        >
          <Text className="text-sm font-semibold text-white">Confirm Changes</Text>
        </TouchableOpacity>
      )}
    </View>
  );
