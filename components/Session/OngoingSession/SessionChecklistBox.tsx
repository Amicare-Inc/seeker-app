...
const SessionChecklistBox = () => {
    const [checklist, setChecklist] = useState<ChecklistItem[]>([
        { id: '1', text: 'Task 1', time: '', checked: false },
    ]);
    ...
    const handleCheck = (id: string) => {
        if (disabled) return; // Prevent checking items if disabled
        setChecklist((items) =>
            items.map((item) =>
                item.id === id
                    ? {
    ...
    return (
        <View className="bg-white/80 rounded-xl mx-5 mt-2 mb-4 px-4 py-3">
            ...
            <View className="flex-row items-center mt-3 mb-3">
                <TouchableOpacity onPress={handleCancelAddItem} className="mr-2" disabled={disabled}>
                    <Ionicons name="ellipse-outline" size={24} color="#bdbdbd" />
                </TouchableOpacity>
                <TextInput
                    ref={newItemInputRef}
