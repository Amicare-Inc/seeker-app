...
const SessionConfirmation = () => {
  ...
  const timeDiff =
    session.startTime
      ? (new Date(session.startTime).getTime() - new Date().getTime()) / (1000 * 60 * 60)
      : null;
  ...
  if (action === "book") {
    ...
    onPrimaryPress = async () => {
      try {
        const newStatus =
          session.status === "pending" ? "declined" : session.status === "confirmed" ? "cancelled" : "";
        if (newStatus) {
          const result = await dispatch(
            updateSessionStatus({ sessionId: session.id, newStatus })
          ).unwrap();
    ...
    if (session.status === "pending") {
      ...
      onPrimaryPress = () => router.push({
        ...
        params: { otherUserId: otherUser.id, sessionObj: JSON.stringify(session) },
      });
    } else if (session.status === "confirmed") {
      if (timeDiff !== null && timeDiff >= 2) {
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="flex-1 justify-center items-center px-4">
        ...
        <View className="flex flex-col space-y-4">
          ...
          >
            <Text className="text-white font-bold text-sm text-center">{primaryButtonText}</Text>
          </TouchableOpacity>
          <TouchableOpacity
            onPress={onBackPress}
            className="bg-black rounded-lg px-6 py-3"
          >
            <Text className="text-white font-bold text-sm text-center">Back to Chat</Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
