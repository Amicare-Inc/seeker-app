...
import { uploadPhotoToFirebase, uploadProfilePhoto } from '@/services/firebase/storage';
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', alignItems: 'center' }}>
        <Text className="text-3xl text-black font-normal mb-3">Add Profile Photo</Text>
        <Text className="text-sm text-gray-500 font-normal mb-6 text-center">
          Tap the circle below to upload your profile photo
        </Text>

        <View className="relative mb-6">
          <TouchableOpacity onPress={handlePhotoSelect} className="w-32 h-32 rounded-full bg-gray-200 border-2 border-gray-300 flex items-center justify-center">
            {localPhotoUri ? (
              <Image source={{ uri: localPhotoUri }} className="w-32 h-32 rounded-full" />
            ) : (
